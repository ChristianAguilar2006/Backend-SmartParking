...
export class LoginUseCase {
    ...
    async ejecutar(email: string, contrasenia: string): Promise<{ token: string, rol: any, nombre: string, idUsuario: any } | null> {
        ...
        if (usuario?.passwordHash == contrasenia && usuario?.email == email) {
            ...
            return { token, rol: (usuario as any).idRol, nombre: (usuario as any).nombre, idUsuario: (usuario as any).idUsuario };
        }
